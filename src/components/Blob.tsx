...
import { generateThemeAccessories } from "../lib/blobGenerator";
import { useTheme } from "../theme";
import { BlobThemeAccessories } from "./BlobThemeAccessories";
import {
  getBodyPath,
  getEyePositions,
  getTopY,
 ...
 *
 * The active theme comes from context rather than props: every call site
 * already has the player's name, and threading a theme through all of them
 * (mountain, ropes, leaderboard, share card, gallery) would be pure noise.
 */
export function Blob({ config, size = 60, state = "idle", className = "" }: BlobProps) {
  ...
  // Deterministic per name + theme — the same player always gets the same
  // themed accessories, and their base blob is unaffected by the theme.
  const themeAccessories = useMemo(
    () => generateThemeAccessories(config.name, theme.id, theme.blobAccessories, accessory),
    [config.name, theme.id, theme.blobAccessories, accessory]
  );

  // Clip path so worn accessories (diaper, bib) hug the body silhouette on
  // every shape instead of each one hand-fitting eight different outlines.
  // Keyed by seed + shape: identical blobs share an identical clip, so a
  // duplicate id is always harmless.
  const clipId = `blob-body-clip-${config.seed}-${body.shape}`;
  ...
  return (
    ...
    >
      ...
      {/* Themed accessories (baby shower etc.) - layered over the base blob */}
      <BlobThemeAccessories
